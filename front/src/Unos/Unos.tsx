...
const KupacUnos = () => {
  ...
  const [recordCount, setRecordCount] = useState(0); // State to hold the count

  useEffect(() => {
    //@ts-ignore
    setRecordCount(fetchDataCount());
  }, []);

  console.log("data count is >> ", recordCount)
  ...
  const fetchDataCount = async () => {
    try {
      const response = await fetch(`http://localhost:2000/countData?tableName=kupci`);
      if (!response.ok) {
